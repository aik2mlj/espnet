...
if [ -z ${data_dir_prefix} ]; then
    log "Root dir for dataset not defined, setting to ${MAIN_ROOT}/egs2/12m"
    data_dir_prefix=${MAIN_ROOT}/egs2/12m
else
    log "Root dir set to ${12m}"
    data_dir_prefix=${12m}
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    mkdir -p ${trg_dir}/12m_test
    mkdir -p ${trg_dir}/12m_dev
    python local/data_prep.py --src "${data_dir_prefix}/12m/test" --dst "${trg_dir}/12m_test"
    python local/data_prep.py --src "${data_dir_prefix}/12m/train" --dst "${trg_dir}/12m_dev"

    for f in wav.scp utt2spk spk2utt; do
        sort ${trg_dir}/12m_test/${f} -o ${trg_dir}/12m_test/${f}
        sort ${trg_dir}/12m_dev/${f} -o ${trg_dir}/12m_dev/${f}
    done

    # make test trial compatible with ESPnet.
    python local/convert_trial.py --trial ${data_dir_prefix}/test_pairs.txt --scp ${trg_dir}/12m_test/wav.scp --out ${trg_dir}/12m_test
