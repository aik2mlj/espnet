...
if [ -z ${data_dir_prefix} ]; then
    log "Root dir for dataset not defined, setting to ${MAIN_ROOT}/egs2/metamidi"
    data_dir_prefix=${MAIN_ROOT}/egs2/metamidi
else
    log "Root dir set to ${metamidi}"
    data_dir_prefix=${metamidi}
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    mkdir -p ${trg_dir}/metamidi_test
    mkdir -p ${trg_dir}/metamidi_dev
    python local/data_prep.py --src "${data_dir_prefix}/metamidi/test" --dst "${trg_dir}/metamidi_test"
    python local/data_prep.py --src "${data_dir_prefix}/metamidi/train" --dst "${trg_dir}/metamidi_dev"

    for f in wav.scp utt2spk spk2utt; do
        sort ${trg_dir}/metamidi_test/${f} -o ${trg_dir}/metamidi_test/${f}
        sort ${trg_dir}/metamidi_dev/${f} -o ${trg_dir}/metamidi_dev/${f}
    done

    # make test trial compatible with ESPnet.
    python local/convert_trial.py --trial ${data_dir_prefix}/test_pairs.txt --scp ${trg_dir}/metamidi_test/wav.scp --out ${trg_dir}/metamidi_test
